...
echo ""
echo "Script start_aprx.sh variables:"
echo ""
echo "\$MYCALL=$MYCALL"
echo "\$LAT=$LAT"
echo "\$LON=$LON"
echo "\$APRSIS_PASSCODE=$APRSIS_PASSCODE"
echo "\$_APRSIS_SERVER=$_APRSIS_SERVER"
echo "\$_APRSIS_SERVICE_PORT_A=$_APRSIS_SERVICE_PORT_A"
...
if [[ "$aprx_enable_service" == "disabled" ]]; then
    ...
    echo "aprx service state is disabled, begin waiting for service state change to \"enabled\""
    ...
    while true; do
        ...
        if [[ "$aprx_enable_service" == "enabled" ]]; then
            echo "aprx service state is now enabled"
            break
...
sed -i "s/server.*/server $_APRSIS_SERVER/" $APRX_CONFIG_FILE
...
echo "APRS-IS server \"$_APRSIS_SERVER\""
