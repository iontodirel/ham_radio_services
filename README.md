...
These containers primarely serve as templates that can easily be modified and repurposed into different applications for APRS and ham radio use. The goal is to have configurability via minimal code changes, minimal scripting, and rather than supporting all possible use cases with knobs, support a set of use cases while providing all flexibility via small code changes.

## Goals

- Easily modifiable and repurposable containers. Easy to repurpose via small code edits.
...
- **Only Linux is currently supported**. This work is targeted at small, cheap and compact Linux embedded devices, running radio services 24/7. 
*Windows and OSX has limitations sharing hadware to Docker containers as they run in a VM.*
- **The --privileged mode**. Compose does not have *hooks* for host scripting, which means we need to run find_devices in the containers, and the containers need hardware access. We could simply run this in the host and only expose to the containers the devices required more granularly, but then we loose the usefulness and convinience of Docker Compose.

## Configuration

These containers are not an E2E application, they are not meant to be taken as is without modifications and change a few flags and in a piece of GUI. These are written for maximum hackability with minimal changes.

If you don't need GPS, or repeater capabilities, or don't have the GPS hardware, simply comment these services in the `compose.yaml` file. Change and update anything else that you don't need. If you need a second direwolf container for RX only, you could add it in the compose file.

If you don't use a Digirig, write another configuration for find_devices to find your own sound card and/or serial port. find_devices should support any USB sound card and USB serial port device.
...
The Docker and Compose caches are lazy, sometimes after you make changes you need to force a cache update with:

`docker compose build --no-cache`

### Running things into the container

This command will share a serial port and the sound subsystem to the Docker container, and run a prompt into it:
...
This will build the gps container, and then run it with hardware access, and open a prompt in the container:
...
- Ubuntu (*have not tested other distros, but probably all works ok as long as Docker is supported*)
- Docker

Update the `.env` file, to set your callsign (`MYCALL`), and APRS-IS passcode (`APRSIS_PASSCODE`).

Use the `find_devices` utility to find your digirig serial port `serial number`, use the find_devices documentation https://github.com/iontodirel/find_devices/blob/main/README.md for more details.
...
Provides the modem services to the Aprx service and to the host. With the `Direwolf` service, we are able to run an RF repeater, and we can also connect external clients like Xastir or YAAC to monitor RF ARPS traffic.

The Direwolf service has access to the host hardware, and exposes ports 8010 and 8020 to the host and outside the host. If desired, these ports can be changed in `.env` by updating the `DIREWOLF_HOST_AGWP_PORT` and `DIREWOLF_HOST_KISS_PORT` variables.

The service shares a private network with the `Aprx` service, so access between `Aprx` and `Direwolf` is not done throught the host.

The service is defined in `compose.yaml`.

### gps

The `gps` service provides GPS data via GPSD.

The `gps` service has access to the host hardware, and exposes ports 8888 to the host. If desired, this ports can be changed in `.env` by updating the `GPS_HOST_PORT` variable.

The service shares a private network with the `Aprx` service, so access between `Aprx` and `gps` is not done throught the host.

A `gps` client like cgps can connect on the host or outside the host using `cgps localhost:8888`. 

### Aprx

Provides repeater capabilities. Uses the `Direwolf` and `GPS` services. Has access to the `14580` and `10152` ports on the host to access the APRS-IS internet service.
...
This container runs `aprx`. The `aprx.conf` is used to configure the `aprx` service and is an example of a *fill-in TX igate*. Change this configuration if you need more from your igate. The `start_aprx.sh` script uses `sed` to setup the callsign, KISS TCP port (from Direwolf), APRS-IS passcode, location etc. which are configured in the `.env` file.
